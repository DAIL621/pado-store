...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [profile, auth, orders, addresses, customerLogs] = await Promise.all([
    supabase.from("profiles").select("id, name, role, created_at, updated_at").eq("id", id).single(), supabase.auth.admin.getUserById(id),
  ...
  if (profile.error) return NextResponse.json({ ok: false, message: profile.error.message }, { status: 404 });
