...
function productKind(seed: ProductDraftSeed) {
  ...
  if (/갈치|고등어|생선|새우|fish|shrimp/.test(text)) return "수산물";
...
export function generateProductDescription(seed: ProductDraftSeed) {
  ...
  return `${origin}에서 준비한 ${name}입니다. ${kind} 특성에 맞춰 선별, 신선 포장, 빠른 출고 기준으로 관리해 고객 식탁까지 안정적으로 보내드립니다.`;
}
...
export function generateAdvantages(seed: ProductDraftSeed) {
  ...
  const kindBenefit: Record<string, string> = {
    전복: "전복 상태에 맞춘 산소·보냉 포장",
    장어: "손질 부담을 줄인 조리 친화 구성",
    조개류: "크기와 신선도 기준 선별",
    수산물: "구이와 조림에 좋은 손질 구성",
    밀키트: "집에서도 간편하게 완성하는 구성",
...
export function generateFAQ(seed: ProductDraftSeed) {
  ...
  return [
    {
      question: "언제 출고되나요?",
      answer: "평일 오후 1시 이전 주문 건은 당일 출고를 기준으로 운영합니다. 산지 상황이나 택배 사정에 따라 변동될 수 있습니다."
    },
    {
...
export function generateCookingGuide(seed: ProductDraftSeed) {
  ...
  const guideByKind: Record<string, Array<{ title: string; description: string; image: string }>> = {
    전복: [
      { title: "전복버터구이", description: "버터와 마늘을 넣고 노릇하게 구우면 전복의 고소한 풍미가 살아납니다.", image: "" },
      { title: "전복죽", description: "쌀과 함께 천천히 끓이면 깊고 부드러운 맛을 즐길 수 있습니다.", image: "" }
    ],
    장어: [
      { title: "장어구이", description: "중불에서 앞뒤로 구운 뒤 소금이나 양념 소스와 함께 드세요.", image: "" },
      { title: "장어덮밥", description: "따뜻한 밥 위에 구운 장어를 올리면 든든한 한 끼가 됩니다.", image: "" }
    ],
    조개류: [
      { title: "숙회", description: "가볍게 데친 뒤 초장이나 간장 소스와 함께 드세요.", image: "" },
      { title: "찜", description: "채소와 함께 쪄내면 시원한 감칠맛을 즐길 수 있습니다.", image: "" }
    ],
    수산물: [
    ...
    선물세트: [
      ...
      { title: "선물 안내", description: "받는 분께 보관법과 추천 조리법을 함께 안내하면 좋습니다.", image: "" }
    ],
    수산물기본: [{ title: `${name} 기본 조리`, description: "상품에 맞는 방식으로 가볍게 익히거나 손질해 즐겨주세요.", image: "" }]
  };

  return guideByKind[kind] || guideByKind.수산물기본;
}
