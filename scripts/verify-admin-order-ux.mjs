...
assert(admin.includes("autoShipOnTracking") && admin.includes('["pending", "paid", "preparing"].includes(status)'), "automatic shipped transition is missing");
assert(admin.includes("송장 저장 시 배송중으로 자동 변경") && admin.includes("useState(true)"), "auto-shipping default toggle is missing");
