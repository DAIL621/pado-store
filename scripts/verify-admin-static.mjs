...
const productByIdApi = read("app/api/admin/products/[id]/route.ts");
...
assert(productsApi.includes("product.created"), "admin product create should write operation logs");
assert(!productsApi.includes(".from(\"products\").delete().eq(\"id\", product.id)"), "admin product create rollback must not physically delete products");
assert(productByIdApi.includes("product.hidden"), "admin product hide should write operation logs");
assert(productByIdApi.includes("product.sale_ended"), "admin product sale end should write operation logs");
assert(productByIdApi.includes("operationState"), "admin product soft delete state should be stored in detail_json");
...
["/admin/members", "/admin/reviews", "/admin/marketing", "/admin/content", "/admin/stats", "/admin/automation", "/admin/ai/images", "/admin/ai/dataset", "/admin/ai/dashboard", "/admin/ai/review"].forEach((href) => {
