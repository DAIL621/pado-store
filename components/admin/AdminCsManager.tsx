...
function CaseDrawer({ detail, onClose, onSaved }: { detail: Detail | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async (body: Record<string, unknown>) => { await fetch(`/api/admin/cs/${current.caseId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...body, userId: current.userId, orderId: current.orderId }) }); setNote(""); await onSaved(); };
  return <div className="admin-cs-drawer-bg"><aside className="admin-cs-drawer" role="dialog"><header><div><span>{current.caseNo}</span><h2>{csTypeLabels[current.type]}</h2></div><span className={`cs-status ${current.status}`}>{csStatusLabels[current.status]}</span><button onClick={onClose}>×</button></header><div className="admin-cs-body"><section><h3>고객·주문</h3><p>{String(detail.customer?.name ?? current.userId)}</p><a href={`/admin/members?q=${current.userId}`}>고객 운영센터</a>{current.orderId && <a href={`/admin/orders?q=${current.orderId}`}>주문관리</a>}</section><section><h3>문의 내용</h3><p>{current.content}</p></section><section><h3>처리 상태·담당자</h3><div className="cs-action-row"><select value={status} onChange={(event) => setStatus(event.target.value as CsStatus)}>{csStatuses.map((item) => <option key={item} value={item}>{csStatusLabels[item]}</option>)}</select><button onClick={() => void save({ status })}>상태 저장</button></div><div className="cs-action-row"><input value={assignee} onChange={(event) => setAssignee(event.target.value)} placeholder={current.assignee} /><button onClick={() => void save({ assignee })}>담당 변경</button></div></section><section><h3>빠른 Workflow</h3><div className="cs-workflows">{[["reship", "재배송 처리"], ["refund", "환불 처리"], ["exchange", "교환 처리"], ["cancel", "취소 처리"]].map(([action, label]) => <button key={action} onClick={() => void save({ workflow: { action, step: "approved", reason: note }, note })}>{label}</button>)}</div></section><section><h3>처리 메모</h3><textarea value={note} onChange={(event) => setNote(event.target.value)} /><button onClick={() => void save({ note })}>기록 추가</button></section><section><h3>처리 기록</h3><ol className="cs-history">{[...detail.history].reverse().map((event, index) => <li key={index}><time>{new Date(String(event.created_at)).toLocaleString("ko-KR")}</time><strong>{String(event.summary)}</strong></li>)}</ol></section></div></aside></div>;
}
