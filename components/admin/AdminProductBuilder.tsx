...
export function AdminProductBuilder({
  ...
  const legacyDetailComplete = detailJson.detailDisplayMode === "legacy" && detailJson.legacyDetailImages.some((image) => image.url.trim());
  ...
  const [slugAvailability, setSlugAvailability] = useState<"idle" | "checking" | "available" | "duplicate">("idle");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/admin/products?slug=${encodeURIComponent(form.slug.trim())}${productId ? `&excludeId=${encodeURIComponent(productId)}` : ""}`);
        const result = await response.json();
        setSlugAvailability(result.available ? "available" : "duplicate");
      } catch {
        setSlugAvailability("idle");
      }
    }, 350);
    ...
    try {
      ...
      if (raw) {
        ...
        if (draft.form) {
          ...
          setSlugManuallyEdited(Boolean(draft.form.slug?.trim()));
        }
        if (draft.options?.length) setOptions(draft.options);
        if (draft.detailJson) setDetailJson(createProductDetailFormValue(draft.detailJson));
  ...
  const update = (key: keyof AdminProductFormState, value: string) => {
    ...
    setForm((current) => {
      ...
      if (mode === "create" && (key === "name" || key === "origin") && !slugManuallyEdited) {
        next.slug = createProductSlug({ name: next.name, origin: next.origin });
      }
      return next;
    });
  ...
  const regenerateSlug = () => {
    ...
    setForm((current) => ({ ...current, slug: createProductSlug({ name: current.name, origin: current.origin }) }));
    setSlugAvailability("checking");
  };
  ...
  const validationWarnings = useMemo(() => {
    ...
    return warnings;
  }, [detailJson, form.badge, form.basePrice, form.name, form.slug, form.subtitle, legacyDetailComplete, loadedDraftSlug, mode, options]);
  ...
  const qualityItems = useMemo(() => {
    ...
    if (legacyDetailComplete) {
      return [
        { label: "기존 상세페이지", value: "적용 완료", done: true, weight: 100, hint: "기존 상세 이미지가 고객 화면에 적용됩니다." },
        { label: "SEO", value: form.slug && form.subtitle.length >= 18 ? "준비" : "보강", done: Boolean(form.name && form.origin && form.slug && form.subtitle.length >= 18), weight: 0, hint: "SEO 정보는 상세페이지 품질 점수와 별도로 관리합니다." }
      ];
    }
    return [
  ...
  }, [detailJson, form.basePrice, form.description, form.name, form.origin, form.slug, form.subtitle, legacyDetailComplete, options]);
  ...
  const launchReadiness = useMemo(() => {
    ...
    const detailReady = legacyDetailComplete || (benefitCount >= 3 && packagingCount >= 3 && faqCount >= 1);
    const checks = [
    ...
    return Math.round((checks.filter(Boolean).length / checks.length) * 100);
  }, [aiDraftLoaded, detailJson, form, legacyDetailComplete, publishMode, qualityScore]);
  ...
  const saveProduct = async (formElement: HTMLFormElement) => {
    ...
    try {
      result = await onSubmit({ form: submittedForm, options: submittedOptions, detailJson, publishMode, reservedAt, slugAutoGenerated: mode === "create" && !slugManuallyEdited });
      updateSubmitDebug({
            ...
            <details className="admin-form-section" data-admin-section="basic" open>
              ...
              <div className="admin-form section-grid">
                ...
                <label>
                  ...
                  <span className={`admin-slug-status ${slugAvailability}`}>
                    {slugManuallyEdited ? "직접 수정" : "자동 생성"}
                    {slugAvailability === "checking" ? " · 확인 중" : slugAvailability === "available" ? " · 사용 가능" : slugAvailability === "duplicate" ? " · 중복됨" : ""}
                  </span>
                  {mode === "create" && <button type="button" className="admin-slug-regenerate" onClick={regenerateSlug}>다시 자동 생성</button>}
                </label>
                <label>
                  산지 <em>필수</em>
